...
var DEFAULT_ROLECHECK_FUNCTION = function(_, __, ___, cb) { return cb(null, false); };
...
module.exports = function(options) {
  ...
  var roleCheckCallback = options.roleCheckCallback || DEFAULT_ROLECHECK_FUNCTION;
  ...
  var middleware = function(req, res, next) {
    ...
    async.waterfall([
      ...
      function roleCheck(decoded, callback) {
        roleCheckCallback(req, dtoken.payload, role, function (err, unauthorized) {
          if (err) {
            callback(err);
          }
          else if (unauthorized) {
            callback(new UnauthorizedError('unauthorized_request', {message: 'Your request is unauthorized.'}));
          } else {
            callback(null, decoded);
          }
        });
